...
var SlackAutoInviter = PromiseObject.create({
	...
	inviteAll: function *($deferred) {
		var typeFormResponse = _.first(yield request.getAsync({url: 'https://api.typeform.com/v0/form/' + this._typeformUID + '?key=' + this._typeformKey + '&completed=true&since=' + this._cubby.get('form-id-since'), json: true}));
